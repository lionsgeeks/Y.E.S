import TransText from "@components/TransText";

const CountrySelect = () => {
    return (
        <>
            <option value="south_africa">
                <TransText
                    en="South Africa"
                    pr="África do Sul"
                    sw="Afrika Kusini"
                    fr="Afrique du Sud"
                    ar="جنوب أفريقيا"
                />
            </option>
            <option value="algeria">
                <TransText
                    en="Algeria"
                    pr="Argélia"
                    sw="Aljeria"
                    fr="Algérie"
                    ar="الجزائر"
                />
            </option>
            <option value="angola">
                <TransText
                    en="Angola"
                    pr="Angola"
                    sw="Angola"
                    fr="Angola"
                    ar="أنغولا"
                />
            </option>
            <option value="benin">
                <TransText
                    en="Benin"
                    pr="Benin"
                    sw="Benin"
                    fr="Bénin"
                    ar="بنين"
                />
            </option>
            <option value="botswana">
                <TransText
                    en="Botswana"
                    pr="Botsuana"
                    sw="Botswana"
                    fr="Botswana"
                    ar="بوتسوانا"
                />
            </option>
            <option value="burkina_faso">
                <TransText
                    en="Burkina Faso"
                    pr="Burquina Faso"
                    sw="Burkina Faso"
                    fr="Burkina Faso"
                    ar="بوركينا فاسو"
                />
            </option>
            <option value="burundi">
                <TransText
                    en="Burundi"
                    pr="Burundi"
                    sw="Burundi"
                    fr="Burundi"
                    ar="بوروندي"
                />
            </option>
            <option value="cabo_verde">
                <TransText
                    en="Cabo Verde"
                    pr="Cabo Verde"
                    sw="Kapu Verde"
                    fr="Cap-Vert"
                    ar="الرأس الأخضر"
                />
            </option>
            <option value="cameroon">
                <TransText
                    en="Cameroon"
                    pr="Camarões"
                    sw="Kamerun"
                    fr="Cameroun"
                    ar="الكاميرون"
                />
            </option>
            <option value="comoros">
                <TransText
                    en="Comoros"
                    pr="Comores"
                    sw="Komori"
                    fr="Comores"
                    ar="جزر القمر"
                />
            </option>
            <option value="congo">
                <TransText
                    en="Congo"
                    pr="Congo"
                    sw="Kongo"
                    fr="Congo"
                    ar="الكونغو"
                />
            </option>
            <option value="congo_dr">
                <TransText
                    en="Congo (the Democratic Republic of)"
                    pr="Congo (República Democrática do)"
                    sw="Kongo (Jamhuri ya Kidemokrasia)"
                    fr="Congo (République Démocratique du)"
                    ar="الكونغو (جمهورية الكونغو الديمقراطية)"
                />
            </option>
            <option value="ivory_coast">
                <TransText
                    en="Ivory Coast"
                    pr="Costa do Marfim"
                    sw="Pwani ya Azur"
                    fr="Côte d'Ivoire"
                    ar="ساحل العاج"
                />
            </option>
            <option value="djibouti">
                <TransText
                    en="Djibouti"
                    pr="Djibuti"
                    sw="Djibouti"
                    fr="Djibouti"
                    ar="جيبوتي"
                />
            </option>
            <option value="egypt">
                <TransText
                    en="Egypt"
                    pr="Egito"
                    sw="Misri"
                    fr="Égypte"
                    ar="مصر"
                />
            </option>
            <option value="eritrea">
                <TransText
                    en="Eritrea"
                    pr="Eritreia"
                    sw="Eritrea"
                    fr="Érythrée"
                    ar="إريتريا"
                />
            </option>
            <option value="eswatini">
                <TransText
                    en="Eswatini"
                    pr="Essuatini"
                    sw="Eswatini"
                    fr="Eswatini"
                    ar="إسواتيني"
                />
            </option>
            <option value="ethiopia">
                <TransText
                    en="Ethiopia"
                    pr="Etiópia"
                    sw="Uhabeshi"
                    fr="Éthiopie"
                    ar="إثيوبيا"
                />
            </option>
            <option value="gabon">
                <TransText
                    en="Gabon"
                    pr="Gabão"
                    sw="Gabon"
                    fr="Gabon"
                    ar="الغابون"
                />
            </option>
            <option value="gambia">
                <TransText
                    en="Gambia"
                    pr="Gâmbia"
                    sw="Gambia"
                    fr="Gambie"
                    ar="غامبيا"
                />
            </option>
            <option value="ghana">
                <TransText
                    en="Ghana"
                    pr="Gana"
                    sw="Ghana"
                    fr="Ghana"
                    ar="غانا"
                />
            </option>
            <option value="guinea">
                <TransText
                    en="Guinea"
                    pr="Guiné"
                    sw="Guinea"
                    fr="Guinée"
                    ar="غينيا"
                />
            </option>
            <option value="equatorial_guinea">
                <TransText
                    en="Equatorial Guinea"
                    pr="Guiné Equatorial"
                    sw="Guinea ya Ikweta"
                    fr="Guinée équatoriale"
                    ar="غينيا الاستوائية"
                />
            </option>
            <option value="guinea_bissau">
                <TransText
                    en="Guinea-Bissau"
                    pr="Guiné-Bissau"
                    sw="Guinea Bissau"
                    fr="Guinée-Bissau"
                    ar="غينيا بيساو"
                />
            </option>
            <option value="kenya">
                <TransText
                    en="Kenya"
                    pr="Quênia"
                    sw="Kenya"
                    fr="Kenya"
                    ar="كينيا"
                />
            </option>
            <option value="lesotho">
                <TransText
                    en="Lesotho"
                    pr="Lesoto"
                    sw="Lesotho"
                    fr="Lesotho"
                    ar="ليسوتو"
                />
            </option>
            <option value="liberia">
                <TransText
                    en="Liberia"
                    pr="Libéria"
                    sw="Liberia"
                    fr="Libéria"
                    ar="ليبيريا"
                />
            </option>
            <option value="libya">
                <TransText
                    en="Libya"
                    pr="Líbia"
                    sw="Libya"
                    fr="Libye"
                    ar="ليبيا"
                />
            </option>
            <option value="madagascar">
                <TransText
                    en="Madagascar"
                    pr="Madagáscar"
                    sw="Madagaska"
                    fr="Madagascar"
                    ar="مدغشقر"
                />
            </option>
            <option value="malawi">
                <TransText
                    en="Malawi"
                    pr="Malawi"
                    sw="Malawi"
                    fr="Malawi"
                    ar="مالاوي"
                />
            </option>
            <option value="mali">
                <TransText
                    en="Mali"
                    pr="Mali"
                    sw="Mali"
                    fr="Mali"
                    ar="مالي"
                />
            </option>
            <option value="morocco">
                <TransText
                    en="Morocco"
                    pr="Marrocos"
                    sw="Moroko"
                    fr="Maroc"
                    ar="المغرب"
                />
            </option>
            <option value="mauritius">
                <TransText
                    en="Mauritius"
                    pr="Maurício"
                    sw="Morisi"
                    fr="Île Maurice"
                    ar="موريشيوس"
                />
            </option>
            <option value="mauritania">
                <TransText
                    en="Mauritania"
                    pr="Mauritânia"
                    sw="Mauretania"
                    fr="Mauritanie"
                    ar="موريتانيا"
                />
            </option>
            <option value="mozambique">
                <TransText
                    en="Mozambique"
                    pr="Moçambique"
                    sw="Msumbiji"
                    fr="Mozambique"
                    ar="موزمبيق"
                />
            </option>
            <option value="namibia">
                <TransText
                    en="Namibia"
                    pr="Namíbia"
                    sw="Namibia"
                    fr="Namibie"
                    ar="ناميبيا"
                />
            </option>
            <option value="niger">
                <TransText
                    en="Niger"
                    pr="Níger"
                    sw="Nijeri"
                    fr="Niger"
                    ar="النيجر"
                />
            </option>
            <option value="nigeria">
                <TransText
                    en="Nigeria"
                    pr="Nigéria"
                    sw="Nijeria"
                    fr="Nigeria"
                    ar="نيجيريا"
                />
            </option>
            <option value="uganda">
                <TransText
                    en="Uganda"
                    pr="Uganda"
                    sw="Uganda"
                    fr="Ouganda"
                    ar="أوغندا"
                />
            </option>
            <option value="central_african_republic">
                <TransText
                    en="Central African Republic"
                    pr="República Centro-Africana"
                    sw="Jamhuri ya Afrika Kati"
                    fr="République centrafricaine"
                    ar="جمهورية أفريقيا الوسطى"
                />
            </option>
            <option value="rwanda">
                <TransText
                    en="Rwanda"
                    pr="Ruanda"
                    sw="Rwanda"
                    fr="Rwanda"
                    ar="رواندا"
                />
            </option>
            <option value="sao_tome_and_principe">
                <TransText
                    en="Sao Tome and Principe"
                    pr="São Tomé e Príncipe"
                    sw="Sao Tome na Principe"
                    fr="São Tomé-et-Príncipe"
                    ar="ساو تومي وبرينسيبي"
                />
            </option>
            <option value="senegal">
                <TransText
                    en="Senegal"
                    pr="Senegal"
                    sw="Senegali"
                    fr="Sénégal"
                    ar="السنغال"
                />
            </option>
            <option value="seychelles">
                <TransText
                    en="Seychelles"
                    pr="Seicheles"
                    sw="Seychelles"
                    fr="Seychelles"
                    ar="سيشيل"
                />
            </option>
            <option value="sierra_leone">
                <TransText
                    en="Sierra Leone"
                    pr="Serra Leoa"
                    sw="Sierra Leone"
                    fr="Sierra Leone"
                    ar="سيراليون"
                />
            </option>
            <option value="somalia">
                <TransText
                    en="Somalia"
                    pr="Somália"
                    sw="Somalia"
                    fr="Somalie"
                    ar="الصومال"
                />
            </option>
            <option value="sudan">
                <TransText
                    en="Sudan"
                    pr="Sudão"
                    sw="Sudan"
                    fr="Soudan"
                    ar="السودان"
                />
            </option>
            <option value="south_sudan">
                <TransText
                    en="South Sudan"
                    pr="Sudão do Sul"
                    sw="Sudan Kusini"
                    fr="Soudan du Sud"
                    ar="جنوب السودان"
                />
            </option>
            <option value="tanzania">
                <TransText
                    en="Tanzania (the United Republic of)"
                    pr="Tanzânia (República Unida da)"
                    sw="Tanzania (Jamhuri ya Muungano wa)"
                    fr="Tanzanie (République-Unie de)"
                    ar="تنزانيا (جمهورية المتحدة)"
                />
            </option>
            <option value="chad">
                <TransText
                    en="Chad"
                    pr="Chade"
                    sw="Chadi"
                    fr="Tchad"
                    ar="تشاد"
                />
            </option>
            <option value="togo">
                <TransText
                    en="Togo"
                    pr="Togo"
                    sw="Togo"
                    fr="Togo"
                    ar="توجو"
                />
            </option>
            <option value="tunisia">
                <TransText
                    en="Tunisia"
                    pr="Tunísia"
                    sw="Tunis"
                    fr="Tunisie"
                    ar="تونس"
                />
            </option>
            <option value="zambia">
                <TransText
                    en="Zambia"
                    pr="Zâmbia"
                    sw="Zambia"
                    fr="Zambie"
                    ar="زامبيا"
                />
            </option>
            <option value="zimbabwe">
                <TransText
                    en="Zimbabwe"
                    pr="Zimbábue"
                    sw="Zimbabwe"
                    fr="Zimbabwe"
                    ar="زيمبابوي"
                />
            </option>
        </>

    )
}

export default CountrySelect;